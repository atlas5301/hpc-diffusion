...
class Attention(nn.Module):
    ...
    def batch_to_head_dim(self, tensor):
        ...
        batch_size = self._batch_to_head_batch_size
        seq_len = self._batch_to_head_seq_len
        dim = self._batch_to_head_dim
        tensor = tensor.reshape(batch_size // head_size, head_size, seq_len, dim)
        tensor = tensor.permute(0, 2, 1, 3).reshape(batch_size // head_size, seq_len, dim * head_size)
        return tensor

    def record_batch_to_head_dim(self, tensor):
        head_size = self.heads
        batch_size, seq_len, dim = tensor.shape
        self._batch_to_head_batch_size = batch_size
        self._batch_to_head_seq_len = seq_len
        self._batch_to_head_dim = dim
        tensor = tensor.reshape(batch_size // head_size, head_size, seq_len, dim)
        tensor = tensor.permute(0, 2, 1, 3).reshape(batch_size // head_size, seq_len, dim * head_size)
        return tensor


    def head_to_batch_dim(self, tensor, out_dim=3, name:str = "none"):
        head_size = self.heads
        batch_size, seq_len, dim = self.records[name]
    ...
    def record_head_to_batch_dim(self, tensor, out_dim=3, name:str = "none"):
        head_size = self.heads
        batch_size, seq_len, dim = tensor.shape
        if not hasattr(self, "records"):
            self.records = dict()
        self.records[name] = (batch_size, seq_len, dim)
        tensor = tensor.reshape(batch_size, seq_len, head_size, dim // head_size)
        tensor = tensor.permute(0, 2, 1, 3)
    ...
    def record_get_attention_scores(self, query, key, attention_mask=None):
        dtype = query.dtype
        if self.upcast_attention:
            query = query.float()
            key = key.float()

        self._query_shape = query.shape
        self._key_shape = key.shape
        baddbmm_input = torch.empty(
            query.shape[0], query.shape[1], key.shape[1], dtype=query.dtype, device=query.device
        )
        beta = 0
        self._baddbmm_input = baddbmm_input
...
class UNetAttnProcessor:
    def __call__(
        ...
        query = attn.head_to_batch_dim(query, name="query")
        key = attn.head_to_batch_dim(key, name="key")
        value = attn.head_to_batch_dim(value, name="value")
